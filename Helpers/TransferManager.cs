using clickfree_Maui.Exceptions;
...
using static clickfree_Maui.Helpers.FileManager;
...
namespace clickfree_Maui.Helpers
{
    public class TransferManager
    {
        ...
        public static void RecursiveDelete(DirectoryInfo baseDir)
        {
            ...
            //if (IsRoot)
            //{
            //    int count = baseDir.GetFiles().Length;
            //    if (count > 0)
            //    {
            ...
            if (filesTobackup != null && filesTobackup.Count > 0)
            {
                ...
                try
                {
                    ...
                    foreach (var file in filesTobackup)
                    {
                        ...
                        {
                            //calc total files size
                            progressInfo.TotalSize += new FileInfo(file).Length;
                            finishedInfo.TotalSize = progressInfo.TotalSize;
                        }
                        catch (DirectoryNotFoundException notFoundException)
                        {
                    ...
                    for (int i = 0, length = filesTobackup.Count; i < length; i++, progressInfo.CurrentPosition++)
                    {
                        ...
                        try
                        {
                            ...
                            if (File.Exists(copyTo)) //skip if the same size of the files
                            {
                                ...
                                if (fileSize != new FileInfo(copyTo).Length)
        ...
        public async Task BackupFromInstagramToSelectedPath(List<clickfree_Maui.Instagram.InstagramManager.MediaResult> from, string to)
        {
            await CancelAsync();
            ...
            mTransferTask = Task.Run(() =>
            {
                ...
                if (from == null)
                {
                    try
                    {
                        var photos = clickfree_Maui.Instagram.InstagramManager.LoadALLPhotos(mCancellationTokenSource).Result;
                        mCancellationTokenSource.Token.ThrowIfCancellationRequested();

                        //var videos = ClickFree.Instagram.InstagramManager.LoadALLVideos(mCancellationTokenSource).Result;
                        //mCancellationTokenSource.Token.ThrowIfCancellationRequested();

                        from = new List<clickfree_Maui.Instagram.InstagramManager.MediaResult>();
                        from.AddRange(photos);
                        //from.AddRange(videos);
                    }
                    catch (OperationCanceledException)
                ...
                TransferFromInstagramToClickFree(to, from);
            });
        }
        private void TransferFromInstagramToClickFree(string to, List<clickfree_Maui.Instagram.InstagramManager.MediaResult> filesToBackup)
        {
            if (filesToBackup != null && filesToBackup.Count > 0)
            {
                TransferProgressInfo progressInfo = new TransferProgressInfo()
                ...
                try
                {
                    ...
                    string rootFolder = Path.GetDirectoryName(to);
                    //int index = 1;
                    foreach(clickfree_Maui.Instagram.InstagramManager.MediaResult backupFile in filesToBackup)
                    {
                        try
                        {
                            Uri file = new Uri(backupFile.Source);
                    ...
                    OnStart(new TransferStartInfo()
                    {
                        TotalFiles = progressInfo.TotalFiles,
                        TotalSize = progressInfo.TotalFiles,
                        CurrentPosition = progressInfo.CurrentPosition,
                    });
            ...
            if (filesToTransfer != null && filesToTransfer.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, length = otherFiles.Count; i < length; i++, progressInfo.CurrentPosition++)
                    {
                        ...
                        if (to.Contains("ClickFree"))
                        {
                            var photos = FileManager.SearchParameters.GetFilters(FileManager.SearchFilterEnum.Photos, ".");
                            ...
                            foreach (var files in file)
                            {
                                ...
                                {
                                    string dest_video = Path.Combine(to + "//Videos//", file.Substring(3, file.Length - 3));
                                    string destFolder = Path.GetDirectoryName(dest_video);
                                    if (!Directory.Exists(destFolder))
                                        Directory.CreateDirectory(destFolder);
                                    Directory.CreateDirectory(destFolder);
                                    progressInfo.FileSource = file;
                                    progressInfo.FileDestination = dest_video;
                                    //progressInfo.CurrentSize += new FileInfo(file).Length;

                                    if (File.Exists(dest_video)) //skip if the same size of the files
                                    {
                                        var fileSize = new ZlpFileInfo(file).Length;
                                        if (fileSize != new ZlpFileInfo(dest_video).Length)
                                            CopyFile(file, dest_video, progressInfo, updated: () =>
                                            {
                                                OnProgress(progressInfo);
                                            });
                                        else
                                ...
                                else
                                {
                                    ...
                                    string dest_photos = Path.Combine(to + "//Photos//", file.Substring(3, file.Length - 3));
                                    string destFolder = Path.GetDirectoryName(dest_photos);
                                    if (!Directory.Exists(destFolder))
                                        Directory.CreateDirectory(destFolder);
                                    progressInfo.FileSource = file;
                                    progressInfo.FileDestination = dest_photos;
                                    //progressInfo.CurrentSize += new FileInfo(file).Length;

                                    if (File.Exists(dest_photos)) //skip if the same size of the files
                                    {
                                        var fileSize = new ZlpFileInfo(file).Length;
                                        if (fileSize != new ZlpFileInfo(dest_photos).Length)
                                            CopyFile(file, dest_photos, progressInfo, updated: () =>
                                            {
                                                OnProgress(progressInfo);
                                            });
                                        else
