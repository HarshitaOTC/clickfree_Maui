...
namespace clickfree_Maui.Helpers
{
    ...
    public static class DriveManager
    {
        ...
        public class DriveStateEventArgs : EventArgs
        {
            #region Properties
        ...
        public class DriverWindow : IDisposable
        {
            #region Nested types
           ...
           ref uint lpBytesReturned, IntPtr lpOverlapped);
            ...
            public static string EjectPenDrive(char driveLetter)
            {
                ...
                if ((long)handle == -1)
                {
                    throw new IOException("Could not open drive " + driveLetter);
                }
                try
                {
                    uint dummy = 0;
                ...
                finally
                {
                    ...
                    {
                        Page = new MessageBoxWindow("Eject", "Click Free", "Dear User thanks for using Click Free, your USB device was ejected successfully.", "")
                        {
                        }
                    };
                    Application.Current.OpenWindow(secondWindow);
                    //DisplayAlert("ClickFree", "Dear User thanks for using Click Free, your USB device was ejected successfully.","Ok");
                    //MessageBox.Show("Dear User thanks for using Click Free, your USB device was ejected successfully.", "ClickFree", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                return "ok";
            }
            [DllImport("kernel32.dll", SetLastError = true)]
        ...
        public static bool CheckAccess(bool showWarningMessage = true)
        {
            ...
            if (!result && showWarningMessage)
                //  DisplayAlert("Could not establish connection with ClickFree.", "Please connect/ re - connect ClickFree to your computer USB port.");
                Console.WriteLine("");
            return result;
        }
                                                ...
                                                using (var volumeSearch = new ManagementObjectSearcher($"select FreeSpace, Size, VolumeName, FileSystem, VolumeSerialNumber from Win32_LogicalDisk where Name='{logical["Name"]}'"))
                                                {
                                                    ...
                                                    using (ManagementObject volume = volumeSearch.First())
                                                    {
                                                        if (volume != null)
                                                        {
                                                                ...
                                                                if (volume["VolumeName"]?.ToString() == null)
                                                                {
                                                                ...
                                                                try
                                                                {
                                                                    ...
                                                                    //if (disk.Model.Contains("Clickfre NS1081 USB Device"))
                                                                    //{
                                                                    //    disks.Add(disk);
                                                                    //}
                                                                    disks.Add(disk);
