...
namespace clickfree_Maui.Facebook
{
    public class FacebookManager
    {
        #region Constants //https://facebook.com/dialog/oauth?client_id=2798931540324100&redirect_uri=https://www.datalogixxmemory.com/&scope=user_photos,user_videos&display=popup";
        ...
        public static string OAuthURL = "https://facebook.com/dialog/oauth?client_id=" + AppID + "&redirect_uri=" + RedirectURL +  "&scope=user_photos,user_videos&display=popup";
        ...
        public static bool CheckNetworkConnection()
        {
            if (!NetworkInterface.GetIsNetworkAvailable())
            {
                ...
                {
                    Page = new MessageBoxWindow("Error", "No Internet connection", "Please connect to internet and try again.", "")
                    {
                    }
                };
        ...
        public static bool CheckAuthorization(INavigationService _navigationService, bool showForm = true)
        {
            ...
            if (showForm)
            {
                ...
                var result = FacebookLoginwindow.Show1(login, _navigationService);

                if (result == FacebookLoginwindow.LoginState.LoggedOut)
                {
                    return FacebookLoginwindow.Show1(true, _navigationService) == FacebookLoginwindow.LoginState.Success;
                }
                else
                {
                    //_navigationService.BackupFacebookMainView();
        ...
        public static bool CheckAuthorization1(INavigationService _navigationService, bool showForm = true, bool logout = false)
        {
            ...
            if (showForm)
            {
                var result = FacebookLoginwindow.Show1(login, _navigationService, logout);

                if (result == FacebookLoginwindow.LoginState.LoggedOut)
                {
                    var result1 = FacebookLoginwindow.Show1(true, _navigationService) == FacebookLoginwindow.LoginState.Success;
            ...
            else if (login)
            {
                ...
                var result = FacebookLoginwindow.Show1(true, _navigationService, logout);
                if (result == FacebookLoginwindow.LoginState.LoggedOut)
                {
                    return FacebookLoginwindow.Show1(true, _navigationService, logout) == FacebookLoginwindow.LoginState.Success;
                }
                else
                {
        ...
        public static void Logout()
            {
                ...
                Settings.Default.FacebookUserName = null;
                Settings.Default.Save();
            }
